...
using Plots
pyplot()  # Switch backend to PyPlot
using LazySets
...
@taylorize function trucktrailer!(du, u, p, t)
    ...
    du[5] = 11.66 + zero(u[5])  # v0 remains constant
    du[6] = 5.15 + zero(u[6])  # dtheta0 remains constant
...
# Safety Property Checks, define unsafe regions
unsafe_theta1 = HalfSpace(Float64[-1, 0, 0, 0, 0, 0], -1.5)
unsafe_x1 = HalfSpace(Float64[1, 0, 0, 0, 0, 0], 0.2)
unsafe_y1 = HalfSpace(Float64[0, 1, 0, 0, 0, 0], 1.5)

# Check safety properties over the solution
tf = tend(sol)

solz_hyper = overapproximate(sol, Hyperrectangle)

unsafe_theta1_check = all([isdisjoint(unsafe_theta1, set(R)) for R in solz_hyper(0.0 .. tf)])
unsafe_x1_check = all([isdisjoint(unsafe_x1, set(R)) for R in solz_hyper(0.0 .. tf)])
unsafe_y1_check = all([isdisjoint(unsafe_y1, set(R)) for R in solz_hyper(0.0 .. tf)])

println("Safety check for theta1: ", unsafe_theta1_check)
println("Safety check for x1: ", unsafe_x1_check)
println("Safety check for y1: ", unsafe_y1_check)

plot(solz_hyper; vars=(1, 2), xlab="x1 (trailer x)", ylab="y1 (trailer y)", lw=0.2, color=:blue, lab="Flowpipe")

# Project unsafe_x1 and unsafe_y1
unsafe_x1_2d = project(unsafe_x1, [1, 2])  # x1 and y1
unsafe_y1_2d = project(unsafe_y1, [1, 2])  # x1 and y1

# Add unsafe regions for `x1` and `y1`
plot!(unsafe_x1_2d; color=:red, alpha=0.3, lab="Unsafe x1")
plot!(unsafe_y1_2d; color=:orange, alpha=0.3, lab="Unsafe y1")
=#
